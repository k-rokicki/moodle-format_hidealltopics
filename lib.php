...
/**
 * This file contains main class for Hide All Topics course format.
 *
 * @since     Moodle 2.0
...
/**
 * Main class for the Hide All Topics course format.
 *
 * @package    format_hidealltopics
 * @copyright  2022 Kacper Rokicki <k.k.rokicki@gmail.com>
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_hidealltopics extends format_base {
    ...
    /**
     * Returns the default section name for the hidealltopics course format.
     *
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_hidealltopics');
        } else {
     ...
     *
     * In case if course format was changed to 'hidealltopics', we try to copy options
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_hidealltopics');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_hidealltopics', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'hidealltopics' allows to set and remove markers in addition to common section actions.
        ...
        $renderer = $PAGE->get_renderer('format_hidealltopics');
        ...
        if ($PAGE->user_allowed_editing()) {
            if (has_capability('moodle/course:sectionvisibility', $context)) {
                if ($hideall && confirm_sesskey()) {
                    set_course_visible($course->id, '0');
                    redirect($PAGE->url);
                }

                if ($showall && confirm_sesskey()) {
                    set_course_visible($course->id, '1');
                    redirect($PAGE->url);
                }
            }
        }
 ...
 * @return inplace_editable
 */
function format_hidealltopics_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            [$itemid, 'hidealltopics'], MUST_EXIST);
 ...
 *
 * @param int $courseid The course to adjust
